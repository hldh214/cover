...
class CoverController extends Controller
{
    ...
    // regex pattern
    const song_detail_pattern = '#song(\?id=|/)(?<id>\d+)#';
    const playlist_pattern = '#playlist(\?id=|/)(?<id>\d+)#';
    const album_pattern = '#album(\?id=|/)(?<id>\d+)#';
    const program_pattern = '#(program|dj)\?id=(?<id>\d+)#';
    const avatar_pattern = '#home\?id=(?<id>\d+)#';
    ...
    public function indexAction()
    {
        if ($this->request->isPost()) {
            ...
            if (preg_match(self::song_detail_pattern, $url, $song_match)) {
                $res = json_decode(file_get_contents(sprintf(self::song_detail_api, $song_match['id']), false, $context), true);
                if (empty($res['songs'])) {
                    $cover = false;
                } else {
                ...
                $res = json_decode(file_get_contents(sprintf(self::playlist_api, $playlist_match['id']), false, $context), true);
                if (empty($res['result'])) {
                    $cover = false;
                } else {
                ...
                $res = json_decode(file_get_contents(sprintf(self::album_api, $album_match['id']), false, $context), true);
                if (empty($res['album'])) {
                    $cover = false;
                } else {
                ...
                $res = json_decode(file_get_contents(sprintf(self::program_api, $program_match['id']), false, $context), true);
                ...
                $res = json_decode(file_get_contents(sprintf(self::avatar_api, $avatar_match['id']), false, $context), true);
